...
class Gallery {
  constructor(debug = null) {
    ...
    this.debug = debug
    this.camera = null  // set during update() for raycasting
    ...
    this.onClick = (event) => {
      // 1. Event delegation: only proceed if the click was on the canvas
      const canvas = event.target
      if (!canvas || !canvas.classList || !canvas.classList.contains('webgl')) return

      if (!this.camera || !this.planes.length) return

      // Calculate mouse position relative to the canvas, not the whole window
      ...
      const hits = raycaster.intersectObjects(this.planes)
      if (hits.length > 0) {
        const url = hits[0].object.userData.projectUrl
  ...
  setPlanes(scene) {
    ...
    this.planeConfig.forEach((plane, index) => {
      ...
      planeMesh.userData.projectUrl = plane.project?.url || null
      planeMesh.userData.projectTitle = plane.project?.title || null
      scene.add(planeMesh)
  ...
  bindPointerEvents() {
    ...
    // Event delegation on the body: catches clicks without needing to re-bind
    // when React destroys and recreates the canvas during navigation
  ...
  update(camera = null, scroll = null) {
    if (!camera) return
    this.camera = camera  // keep reference for raycasting in onClick
