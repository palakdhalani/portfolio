...
class Scroll {
  constructor(camera, gallery, debug = null) {
    ...
    this.onWheel = (event) => {
      ...
      const attemptedScroll = this.scrollTarget + normalizedWheelDelta * scrollDirection
      ...
      if (this.useScrollBounds) {
        ...
        // Let scroll fall through if we are at bounds
        if (this.scrollTarget <= minimumScroll && attemptedScroll < minimumScroll) return;
        if (this.scrollTarget >= maximumScroll && attemptedScroll > maximumScroll) return;
      }
    ...
    this.onTouchMove = (event) => {
      ...
      const attemptedScroll = this.scrollTarget + deltaY * this.touchScrollSpeed * (this.invertScroll ? -1 : 1)
  ...
  bindEvents() {
    // No wheel/touch binding — the camera is driven by Lenis scroll position
    // via the DepthGallery React component.
  }
  ...
  update() {
    ...
    // CUSTOM PORTFOLIO INTEGRATION: Sync scrollTarget with window scroll
    ...
    if (wrapper) {
      ...
      const maxScrollY = rect.height - windowHeight
      let progress = -rect.top / maxScrollY
      progress = THREE.MathUtils.clamp(progress, 0, 1)

      this.scrollTarget = minimumScroll + progress * (maximumScroll - minimumScroll)
    }
